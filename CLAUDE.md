...
- 2026-09-07 - Dockerfile frontend stage `node:22-alpine` -> `node:24-alpine`. Node 22 is past active LTS; 24 is the current active LTS and satisfies Vite 8's `^20.19 || >=22.12`. Node is still pinned only in the Dockerfile - there is no `.nvmrc`, no `engines` field, and no `setup-node` in CI (the PR workflow builds the frontend only inside the Docker build), so a local `npm run build` on a different Node major rewrites every bundle content hash and produces a large diff that is not a real change. Rebuild the tracked `wwwroot` on Node 24 only. The image no longer receives the committed bundles at all (`src/CountOrSell.Api/wwwroot/` is in `.dockerignore`): the backend stage used to copy them in via `COPY src/` and then merge the fresh build over the top, which shipped one orphaned unreferenced chunk per bundle whose content hash had moved - 18 of 35 at the time of the Node bump.
