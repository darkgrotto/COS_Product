...
- [ ] **Package host migration to `packages.countorsell.com`**
  - State: `UpdateSource` allowlists `www.countorsell.com` plus two package hosts - the stable `packages.countorsell.com` and, transitionally, the storage account the website manifest still emits (`cosadminstoreprod.blob.core.windows.net`). Pinning hosts in source is what makes the allowlist an SSRF guard, but pinning a storage account name means a Backend account move breaks every deployment until a new Product build ships; the stable hostname exists to remove that coupling. `packages.countorsell.com` is not yet in DNS.
  - Next (outside this repo, in order): stand the hostname up in front of the storage account with TLS - Azure Storage does not serve HTTPS on a custom domain, so a bare CNAME to the blob endpoint is not enough. Either a Cloudflare-proxied CNAME to `cosadminstoreprod.blob.core.windows.net` with an Origin Rule setting the Host header to that same blob hostname (Blob resolves the account from `Host`, so it 400s without the override), or Azure Front Door with the custom domain and origin host header set. Container names stay the first path segment either way, so the Backend swaps only the host when generating manifests. Then drop `LegacyPackageStorageHost` from `UpdateSource` and its test cases. Do not make the allowlist configurable.
