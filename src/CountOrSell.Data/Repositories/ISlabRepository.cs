...
public interface ISlabRepository
{
    ...
    Task<(List<SlabEntry> Items, int Total)> GetByUserPagedAsync(Guid userId, CollectionFilter? filter, string? sort, string? sortDir, int page, int pageSize, CancellationToken ct = default);
