...
public interface ISerializedRepository
{
    ...
    Task<(List<SerializedEntry> Items, int Total)> GetByUserPagedAsync(Guid userId, CollectionFilter? filter, string? sort, string? sortDir, int page, int pageSize, CancellationToken ct = default);
